# -*- coding: utf-8 -*-
"""
Created on Sat Apr 17 14:06:24 2021
...
def parse_one_xml(xml_file, names_map):
    tree = ET.parse(os.path.join('./dataset/VOCdevkit/VOC2007/Annotations', xml_file))
    root = tree.getroot()
    filename = root.find('.//filename').text
    filepath = os.path.join('./dataset/VOCdevkit/VOC2007/JPEGImages', filename)
...
def main():
    ...
    train_val_split = {}
    with open('./dataset/VOCdevkit/VOC2007/ImageSets/Main/train.txt') as train_fp:
        lines = train_fp.read().splitlines()
        for line in lines:
            train_val_split[line] = 'train'
    with open('./dataset/VOCdevkit/VOC2007/ImageSets/Main/val.txt') as val_fp:
        lines = val_fp.read().splitlines()
        for line in lines:
            train_val_split[line] = 'val'
    with open('./dataset/VOCdevkit/VOC2007/ImageSets/Main/test.txt') as val_fp:
        lines = val_fp.read().splitlines()
        for line in lines:
            train_val_split[line] = 'test'

    with open('./dataset/voc_2007_names.txt') as names_fp:
    ...
    val_annotations = []
    test_annotations = []
    for xml_file in os.listdir('./dataset/VOCdevkit/VOC2007/Annotations'):
        image_id = xml_file[:-4]
